        ...
        <div class="w-full px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-1">
                    ...
                    <x-card.dashboard caption="Property Name" value="{{ $tenant->room->property->name ?? 'N/A' }}" />

                    <x-card.dashboard caption="Room Number" value="{{ $tenant->room->code ?? 'N/A' }}" />

                    <x-card.dashboard caption="Rent Due"
                        value="{{ number_format(optional($latestBill)->amount_due ?? 0, 2) . ' PHP' }}" footer="true"
                        footerCaption="Due Date"
                        footerDate="{{ optional(optional($latestBill)->due_date)->format('M d, Y') ?? 'N/A' }}" />

                    <x-card.dashboard caption="Outstanding Balance"
                        value="{{ number_format($outstandingBalance ?? 0, 2) . ' PHP' }}" footer="true"
                        footerCaption="Last Due Date"
                        footerDate="{{ optional(optional($latestOverdueBill)->due_date)->format('M d, Y') ?? 'N/A' }}" />

                    <x-card.dashboard caption="Months Stayed" value="{{ $monthsStayed ?? 0 }}" />
                </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 table-fixed">
                                    <thead>
                                        <tr>
                                            ...
                                            <x-table.header :chevron="false" class="w-[17%] whitespace-nowrap">Room
                                                Code</x-table.header>
                                            <x-table.header :chevron="false"
                                                class="w-[40%] whitespace-nowrap">Title</x-table.header>
                                            <x-table.header :chevron="false" class="w-[20%] whitespace-nowrap">Date
                                                Issued</x-table.header>
                                            <x-table.header :chevron="false"
                                                class="w-[15%] text-right whitespace-nowrap"
                                        ...
                                        @foreach ($announcements as $index => $announcement)
                                            <x-table.row>
                                                ...
                                                <td
                                                    ...
                                                    {{-- Added overflow-hidden for long titles --}}
                                                    {{ $announcement->title }}
                                                </td>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 table-fixed">
                                    <thead>
                                        <tr>
                                            ...
                                            <x-table.header class="w-[15%] text-right whitespace-nowrap"
                                                :sortable="false">Details</x-table.header>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
